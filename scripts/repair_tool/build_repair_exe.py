"""Build script for the standalone 2.2.0 name-collision repair tool.

Usage:
    python scripts/repair_tool/build_repair_exe.py

Produces a single console EXE at dist/SmartCitizen-NameCollisionFix.exe.
Deliberately does not reuse scripts/build/build_exe.py: that one bundles
the full GUI (assets, languages, docs, the enhancements generator) for the
main app. This tool only needs QtCore (via AppSettings' QSettings calls)
and the parser/merger/utils packages -- no QtWidgets, no bundled docs or
language files -- so a separate, much smaller build keeps it that way.
"""
import os
import shutil
import sys
...
# repair_name_collisions.py imports `from src.utils...` (repo-root-relative,
...
entry_point = os.path.join(project_dir, 'repair_name_collisions.py')
...
work_dir = os.path.join(root_dir, 'build', 'repair_tool')
if os.path.exists(work_dir):
    shutil.rmtree(work_dir)

args = [
    entry_point,
    '--name', 'SmartCitizen-NameCollisionFix',
    '--onefile',
    '--console',
    '--paths', root_dir,
...
print("Building SmartCitizen-NameCollisionFix.exe...")
PyInstaller.__main__.run(args)
print("\nDone: dist/SmartCitizen-NameCollisionFix.exe")
